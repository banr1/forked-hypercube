...
def compute_extrema(mu_scaled, d0):
    ...
    v2 = (
        mu_scaled * sum_ld_cd * sum_ld_cd
    ) - sum_ld_cd2 * powcc  # multiply right term by powcc since mu is scaled
    v3 = (
        mu_scaled * sum_ld - powcc
    ) * sum_ld  # multiply right term by powcc since mu is scaled
    if v3 == 0:
        return (0, 0)
    v4 = v2 / v3
    discr = v1 - v4
    ...
    for i in range(d0 + 1):
        ...
        if opt_mu1_scaled[i] < 0:
            var1_neg = True
        full_cost1 += opt_mu1_scaled[i] * (
            layer_sizes[i] * compute_cost(i)
        )  # the cost is temporarily scaled up by sum_ld
        checksum_cost += (
            opt_mu1_scaled[i]
            * layer_sizes[i]
            * (compute_cost(i) + compute_checksum_cost(d0 - i))
        )  # the cost is temporarily scaled up by sum_ld
    ...
    for v in range(
        ...
        for w in range(min_w, max_w + 1):  # no need in too expensive verifiers
            ...
            if hcube_size_log2 < sec_level:
                continue
            mu_scaled = ceil(
                pow(2, hcube_size_log2 - sec_level)
            )  # the loss we can tolerate
            (c, d0, cc, dc) = compute_L2_lower_bound(
                v, w, mu_scaled
            )  # compute lower bound and the d0 value
            if c < best_cost - 0.5:
                best_cost = c
                best_w = min_w
                print(
                    ...
                    d0,
                    " mu_log2=",
                    "{0:0.2f}".format(hcube_size_log2 - sec_level),
                    file=open(fname, "a+"),
                )  # Th Ver cost w/o checksum:  683.23  d_0= 775  bit loss= 0.4 chain len= 54
...
def test_tcr():
    ...
    chain_len = 6
    loss = 41.4571
    (c, d0) = compute_max_entropy_lower_bound(
        chains, chain_len, pow(2, loss)
    )  # compute lower bound and the d0 value
    print("Hypercube: ", chains, " chains of length ", chain_len)
    print("Cost min-entropy lower bound is ", c, " d0=", d0)
    print(
        "Size w/o checksum ", (chains) * 4 * 7, " bytes"
    )  # assuming 28 bytes per chain
